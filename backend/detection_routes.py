...
def run_detection_job(db: Session) -> dict:
    """
    The actual detection pipeline, extracted as a plain function (not a
    route) so it can be called identically from:
      - POST /detect (manual trigger, e.g. for a live demo)
      - the background scheduler (automatic, periodic)
    Both paths get exactly the same tested behavior — no drift between
    "the thing that runs automatically" and "the thing I demo live."
    """
...
def run_detection(db: Session = Depends(get_db)):
    """Manual trigger — same pipeline the scheduler runs automatically, callable on demand (e.g. for a live demo)."""
    return run_detection_job(db)
...
def list_detections(host: str | None = None, severity: str | None = None, limit: int = 100, db: Session = Depends(get_db)):
