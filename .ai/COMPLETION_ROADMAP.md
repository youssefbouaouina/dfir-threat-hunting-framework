...
|---|---|---|---|---|---|
| F1 ✅ (`5afca4d`) | Async ingest queue (Redis/RabbitMQ) + workers | new `workers/`, `docker-compose.yml`, `main.py` | high | high | `/ingest` returns 202; workers persist |
| F2 ✅ (`46310fb`) | Correlation engine: `incidents` + `incident_detections` | `models.py`, new migration, `services/correlation_service.py` | high | high | detections group into incidents with ATT&CK chains |
...
- Phases A + B + C + D **complete** (D3/D4 in `9ae3c33`, D1 in `af5b401`, D2 in `132b873`). F-series: **F1 (async queue, `5afca4d`), F2 (correlation, `46310fb` + wiring fix `b260b55`), F3 (retention, `ccf62a8`), F4 (RBAC/audit, `c503503`) done**; F5 (notifications) next, then F6–F8.
