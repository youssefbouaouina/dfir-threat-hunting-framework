...
- **S1. Leaked API keys on disk.** ~~`detection/.env.txt`~~ (deleted, user-approved) and ~~`backend/.env.txt`~~ (deleted) contained real AbuseIPDB/OTX/URLhaus keys; also present in git history. **Deletion done; key rotation on provider dashboards is a user action still required.** *(Critical)* — Note: a full-history gitleaks scan still flags the 3 keys in `detection/.env.txt` (commit `d274bb5`); CI's range-based scans don't reach that ancient commit, but the keys should be rotated and, ideally, the history scrubbed (`git filter-repo`).
...
- **S9. Gitleaks doc-example allowlist (`demo-*` keys).** `.gitleaks.toml` (repo root) extends the default rules and allow-lists the documented demo/example keys (`demo-admin-key`, `demo-agent-key-1`, `my-super-secret-admin-key`, `agent-key-ubuntu`, `agent-key-win10`, `a-long-random-string-for-signing-tokens`) that appear in old doc commits (curl examples, commit `3d4d153`/`bf49bd1`). These are not real secrets (auth is off by default; startup rejects placeholders when auth is on; current docs use `<angle-bracket>` placeholders). The allow-list keeps the gitleaks gate green for historical scan ranges; docs were also sanitized to placeholders. **Review the allow-list if a real secret ever looks like one of these values.**
