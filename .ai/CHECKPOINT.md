...
>
> **F-series progress (same session, Phases 4–5):** F1 ✅ async Redis ingest queue (commit `5afca4d`): `backend/ingest_queue.py` fail-soft enqueue/dequeue; `/ingest` returns 202 + `accepted=true,queued=N` when `INGEST_QUEUE_URL` set (sync 200 default); `workers/ingest_worker.py` drains via the same `ingest_service.ingest_artifacts` (batch_id idempotency preserved); docker-compose redis + worker services; CI `agent-e2e` now exercises the full async loop. 9 backend + 1 collector tests. F2 ✅ correlation engine (commit `46310fb`): `Incident` + `IncidentDetection` models (migration `e19d4f2a7c10`), `services/correlation_service.py` (campaigns = same rule ≥2 hosts; chains = ≥2 techniques one host; severity escalation; idempotent signature-keyed `recompute_incidents` preserving triage), `/incidents` router (list/summary/detail/recompute/PATCH), `run_detection_job` recomputes after every run. 10 tests. (Also fixed a leftover: `app.include_router(incident_router)` wiring commit `b260b55`.) F3 ✅ retention/archival (commit `ccf62a8`): `services/retention_service.py` ages out rows per `RETENTION_*_DAYS` into monthly JSONL under `RETENTION_ARCHIVE_DIR` + optional OpenSearch sink (`OPENSEARCH_URL`, fail-soft) + batch delete; `retention_sweep` scheduler job; `GET /retention/status` + `POST /retention/run` (audited). 9 tests. F4 ✅ RBAC/team scoping + immutable audit (commit `c503503`): roles `admin`/`analyst`/`viewer` (`ADMIN_API_KEY` + `ANALYST_API_KEYS`/`VIEWER_API_KEYS` in `key@team` form), `issue_token`/`current_user`/`require_role` in `security.py`; `Endpoint.team` (migration `4a1f2c9d3b70`) + `scoped_hosts` applied across artifacts/detections/runs/summary/incidents/endpoints (empty team host list now hides everything); audit SHA-256 hash chain (`prev_hash`/`record_hash`) + `verify_audit_chain` + `GET /audit-logs/verify`. 14 tests. **Backend now 115 tests, ruff + mypy clean (46 files).**
>
> **F5 + dashboard session update (2026-08-04):** F5 ✅ **notifications** (`services/notification_service.py` — webhook + SMTP email, `NOTIFY_*` env, fail-soft; fired from `run_detection_job` severity threshold + `mark_offline_stale` offline alert). **Host criticality severity factor** — `Endpoint.criticality` (`low`/`standard`/`important`/`critical`, migration `5f0a1c2d9b73`) amplified into correlation severity scoring (`important` → +1, `critical` → +2 rank bump, capped at critical). **Queue-driven detection worker** (`workers/detection_worker.py`) — `run_one_sweep`/`run_loop` calling the shared `run_detection_job`; guards None results. **Scan-all + per-endpoint report** — `POST /endpoints/scan-all` (audited `queue_collection_all`) and `GET /endpoints/{id}/report` (artifacts/detections/incidents/run-history aggregates). **Dashboard rewritten as a brutalist technical report** (print-style, near-black ink, one `#d7263d` accent, offset shadows, mono data; new Incidents + Endpoint Report views, SCAN ALL control, criticality badges, bearer-token auth). **Collector enroll helpers** `scripts/enroll_agent.sh`/`.ps1` (venv + enroll + daemon). Docker: compose now passes `NOTIFY_*` env + commented `detection-worker` service block. **Validation:** backend 133 tests (+18 in `test_phase4_f5.py`), collector 10, ruff + mypy clean, docker build + compose stack verified, dashboard views smoke-tested over live HTTP. Docs updated (ROADMAP, PROJECT_OVERVIEW, PROJECT_SUMMARY, .ai/*). *Unpushed: 8+ commits (F1–F4 + docs) — pushing is the next mission step, keep `GG.md` uncommitted.*
>
> **F6–F8 session update (2026-08-04 continuation, Phase 5 complete):** F6 ✅ **pySigma backend** — `backend/sigma_engine.py` real pySigma backend (typed condition tree, `1 of`/`all of` selectors, NOT filters, `|re`/`|cidr`/`|gte` modifiers, `SigmaString` case-insensitive regex; `FIELD_MAP` + `LOGSRC_TO_ARTIFACT`); 6 native rules in `sigma_rules/native/`; `run_detection_job` now runs both legacy + pySigma engines; `services/sigma_service.py` imports SigmaHQ (local dir or shallow git clone) with filter + dedup-by-id; `GET /sigma/status|rules` + `POST /sigma/refresh` (admin, audited); `pysigma>=1.5.0`. F7 ✅ **IOC feed automation + STIX/TAXII** — `Ioc` model (migration `6f7a1b2c3d4e`, unique `(value,ioc_type,source)`); `services/intel_service.py` (Feodo/URLhaus/MalwareBazaar/OTX, idempotent upsert, fail-soft per feed, scheduler + `POST /iocs/refresh`); `GET /iocs` + `/iocs/status` + `/iocs/export/stix` (STIX 2.1 bundle); minimal TAXII 2.1 server in `taxii_routes.py`. F8 ✅ **HA/perf** — `services/circuit_breaker.py` per-feed breakers (closed/open/half-open, `IOC_FEED_*` env, reset via `/iocs/breakers/reset`); DB pooling env knobs in `database.py`; composite-index migration `7a8b1c2d3e4f`; `StatsSnapshot` + `services/stats_service.py` materialized stats (`GET /stats/summary`, `POST /stats/recompute`, scheduler job); `k8s/` manifests (3-replica backend + HPA 3–10 + PDB + ingest worker + Postgres StatefulSet). **Validation:** backend **179 tests** (+46: 16 sigma engine, 17 intel service, 13 F8), collector 10, ruff + mypy clean (64 source files), both new migrations verified against a clean scratch DB. *Docs updated; F6–F8 not yet committed/pushed — next step is commit + push + retag `v1.0.0`.*
...
## Current maturity
- Phases 1–3 of the 5-phase roadmap **implemented and committed on `youssef`**; roadmap Phases A + B (critical/high hardening), C (medium), D (low), and **F1–F8 (Phases 4–5: async queue, correlation, retention, RBAC/audit, notifications + host criticality, pySigma, IOC feeds + STIX/TAXII, k8s/HA) are all complete**.
- 179 backend pytest tests + 10 collector pytest tests; ruff clean; mypy clean (gradual); CI (GitHub Actions) gates lint+mypy+pytest+pip-audit+gitleaks, plus a containerized agent→backend e2e job.
...
- No production deployment and no long-running soak yet; user-side provider key rotation still pending.
...
- Notifications are fire-and-forget (fail-soft) — no retry/backoff, no delivery receipts; SMTP needs real SMTP creds to exercise.
- YARA only runs at collection time on the agent (no backend re-scan); embedded results only.
- Scheduler is in-process (single point of failure; detection worker exists but the queue-driven scheduling wiring is commented out in compose).
